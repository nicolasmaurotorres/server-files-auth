...
func (p *parser) GetUserAndCategory(r *http.Request) (UserLoginRequest, int, error) {
	...
	if govalidator.IsNull(user.Email) {
		return user, 0, errors.New(ERROR_BAD_FORMED_EMAIL_EMPTY)
	}
	if govalidator.IsNull(user.Password) {
		return user, 0, errors.New(ERROR_BAD_FORMED_PASSWORD)
	}
	if !GetDatabaseInstance().ExistsEmail(user.Email) {
		return user, 0, errors.New(ERROR_EMAIL_NOT_EXISTS)
	}
	userDBO, errUser := GetDatabaseInstance().GetUserByEmail(user.Email)
	if errUser != nil {
		return user, 0, errUser
	}
	return user, userDBO.Category, nil
}
